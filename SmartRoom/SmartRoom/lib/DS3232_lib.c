...
uint8_t __RTC_Buffer[__RTC_Buffer_Length];
	...
	RTC_Status_t __RTC_Status = RTC_OK;
#endif

//---------------------------//

//----- Prototypes ----------------------------//
static uint8_t FlagWrite(uint8_t Address, uint8_t Flag, uint8_t Value);
static uint8_t FlagRead(uint8_t Address, uint8_t Flag);
...
//Write byte to RTC.
void RTC_WriteByte(uint8_t Address, uint8_t Data)
{
...
void RTC_WriteBlock(uint8_t StartAddress, uint8_t *Data, uint8_t Length)
{	
	uint8_t i;
	#if (RTC_Error_Checking != 0)
		uint8_t status;
...
void RTC_ReadBlock(uint8_t StartAddress, uint8_t *Data, uint8_t Length)
{
	uint8_t i;
	#if (RTC_Error_Checking != 0)
		uint8_t status;
...
uint8_t RTC_32KHZ(uint8_t Enable)
{
	return (FlagWrite(__RTC_Address_Status, __RTC_EN32KHZ, Enable));
}

void RTC_TemperatureSetRate(TCXO_Sample_Rate_t Rate)
{
	//Read control register
...
double RTC_TemperatureRead(uint8_t Force)
{
	union
	{
		uint16_t i;
...
static uint8_t FlagWrite(uint8_t addr, uint8_t flag, uint8_t enable)
{
	uint8_t reg, r;
...
static uint8_t FlagRead(uint8_t addr, uint8_t flag)
{
